...
const PastelMobileNav = memo(({ logout }) => {
  return (
    <div className="lm-mobile-only" style={{ height: 60, padding: "0 16px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(247,232,240,0.92)", borderBottom: "1px solid rgba(242,196,208,0.4)", backdropFilter: "blur(20px)", zIndex: 100 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg, #ffaad8, #cc88ff)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, boxShadow: "0 2px 8px rgba(255,130,200,0.3)" }}>🌸</div>
        <span style={{ fontSize: 18, fontWeight: 900, letterSpacing: 1, background: "linear-gradient(90deg, #e060b0, #a060e0)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", fontFamily: "'DM Sans', sans-serif" }}>ORBIT</span>
      </div>
      <button onClick={logout} style={{ padding: "6px 14px", background: "rgba(255,120,170,0.1)", border: "1px solid rgba(255,120,170,0.4)", borderRadius: 20, color: "#e0507a", fontSize: 10, fontWeight: 800 }}>LOGOUT</button>
    </div>
  );
});

const PastelMobileDash = memo(({ 
  nexuses, isNexusesLoading, setSelectedNexus, selectedNexus,
  users, setSelectedUser, selectedUser, nexusUnread, setNexusActionView,
  hiddenNexuses, toggleHide, activeTab
}) => {
  return (
    <div className="lm-mobile-only" style={{ flex: 1, display: "flex", flexDirection: "column", background: "#fdf8fc", overflowY: "auto", position: "relative" }}>
        <BgClouds />
        <Floaties />
...
const PastelMobileTabBar = memo(({ activeTab, setActiveTab }) => {
  const tabs = [
    { id: "orbits", label: "CLUBS", icon: "🌸" },
    { id: "chats", label: "BESTIES", icon: "💬" },
    { id: "settings", label: "MAGICAL", icon: "⚙️" }
  ];
  const navigate = useNavigate();

  return (
    <div className="lm-mobile-only" style={{ height: 70, display: "flex", background: "rgba(247,232,240,0.92)", borderTop: "1px solid rgba(242,196,208,0.4)", backdropFilter: "blur(20px)", zIndex: 100, paddingBottom: "env(safe-area-inset-bottom)" }}>
      {tabs.map(t => (
        <button
          key={t.id}
          onClick={() => {
          ...
          style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4, background: "none", border: "none", cursor: "pointer", color: activeTab === t.id ? "#e060b0" : "rgba(200,169,212,0.6)", transition: "all 0.3s" }}
        >
          <span style={{ fontSize: 20, filter: activeTab === t.id ? "drop-shadow(0 0 8px rgba(224,96,176,0.3))" : "none" }}>{t.icon}</span>
          <span style={{ fontSize: 8, fontWeight: 900, letterSpacing: 1, fontFamily: "'DM Sans', sans-serif" }}>{t.label}</span>
        </button>
      ))}
    </div>
  );
...
const Sidebar = memo(({ 
  sidebarRef, nexuses, isNexusesLoading, setSelectedNexus, selectedNexus, 
  users, setSelectedUser, selectedUser, nexusUnread, setNexusActionView, 
  hiddenNexuses, toggleHide, forcedTab = null 
}) => {
  const [activeTab, setActiveTab] = useState(forcedTab || "orbits");
  const play = (s) => soundManager.playSound(s);
  const navigate = useNavigate();
        ...
        <div style={{ position: "relative", width: "100%", maxWidth: 1100, height: "100%", maxHeight: 650 }}>
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", background: "rgba(255,255,255,0.65)", backdropFilter: "blur(25px)", borderRadius: 40, border: "3px solid rgba(255,255,255,0.9)", boxShadow: "0 25px 60px rgba(230,190,220,0.5)", overflow: "hidden", zIndex: 10 }}>
            ...
            <div style={{ width: 300, background: "linear-gradient(180deg, rgba(255,240,245,0.6) 0%, rgba(245,230,255,0.6) 100%)", padding: "40px 30px", display: "flex", flexDirection: "column", gap: 12, borderRight: "3px solid rgba(255,255,255,0.7)" }}>
              ...
              {sections.map(s => (
                <button key={s.id} onClick={() => setActiveSection(s.id)} style={{ padding: "16px 20px", background: activeSection === s.id ? "rgba(255,255,255,0.9)" : "transparent", border: "none", borderRadius: 20, color: activeSection === s.id ? "#a855f7" : "#d060a8", fontSize: 16, fontWeight: 800, textAlign: "left", display: "flex", alignItems: "center", gap: 16, cursor: "pointer", transition: "all 0.2s", boxShadow: activeSection === s.id ? "0 4px 15px rgba(200,150,180,0.2)" : "none" }}>
                  <span style={{ fontSize: 22 }}>{s.id === 'profile' ? '🍬' : '🎨'}</span> {s.label}
                </button>
              ))}
            ...
            {/* Content Area */}
            {activeSection && (
            <div style={{ flex: 1, padding: "50px 60px 100px", overflowY: "auto", position: "relative" }}>
              <h2 style={{ fontSize: 36, fontWeight: 900, color: "#a855f7", marginBottom: 30, textShadow: "0 2px 10px rgba(168,85,247,0.2)" }}>
                {sections.find(t => t.id === activeSection)?.label}
              </h2>
               ...
               {activeSection === "appearance" && (
                 <div>
                   ...
                   <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                     {THEMES.map(th => (
                       <div key={th} onClick={() => { setDraftTheme(th); handleSave(th); }} style={{ padding: "20px", background: draftTheme === th ? "linear-gradient(135deg, #f472b6, #c084fc)" : "rgba(255,255,255,0.6)", borderRadius: 25, border: draftTheme === th ? "none" : "3px solid rgba(244,114,182,0.3)", color: draftTheme === th ? "#fff" : "#a855f7", fontSize: 16, fontWeight: 800, cursor: "pointer", transition: "all 0.2s", boxShadow: draftTheme === th ? "0 8px 20px rgba(192,132,252,0.4)" : "none", textAlign: "center" }}>
                         {THEME_LABELS[th] || th}
                      </div>
                    ))}
                  </div>
