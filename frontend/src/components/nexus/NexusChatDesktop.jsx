...
function Toast({t,msg,onDone}) {
  ...
  return (
    <div style={{position:"fixed",top:24,left:"50%",transform:"translateX(-50%)",zIndex:9999,background:t.header,border:`1.5px solid ${t.acc}`,borderRadius:12,padding:"12px 24px",color:t.txt,fontFamily:t.font,fontSize:14,boxShadow:`0 8px 32px ${t.glow}`,animation:"notifSlide 3.5s ease forwards",whiteSpace:"nowrap"}}>
...
function InfoPanel({t,group,setGroup,onClose,addToast}) {
  const [tab,setTab]=useState("overview"); // overview | members | media | settings | danger
  ...
  return (
    <div style={{width:340,borderLeft:`1px solid ${t.border}`,background:t.panel,display:"flex",flexDirection:"column",height:"100%",animation:"slideInRight .3s ease",flexShrink:0,position:"relative",overflow:"hidden"}}>
      <ParticleCanvas t={t} style={{opacity:.3}}/>

      {/* Header */}
      <div style={{padding:"18px 16px 14px",borderBottom:`1px solid ${t.border}`,background:t.header,position:"relative",zIndex:1,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
          <button onClick={onClose} style={{background:"none",border:"none",color:t.txt2,cursor:"pointer",padding:4,borderRadius:7,display:"flex"}}><Ico d={I.back} size={18} stroke={t.txt2}/></button>
          <div style={{flex:1,color:t.txt,fontSize:15,fontWeight:700,fontFamily:t.font,letterSpacing:".03em"}}>Group Info</div>
        </div>
        {/* Avatar + name */}
        <div style={{textAlign:"center",padding:"8px 0 4px"}}>
          <div style={{width:72,height:72,borderRadius:"50%",background:t.avatar,border:`3px solid ${t.acc}`,margin:"0 auto 10px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:30,boxShadow:`0 0 24px ${t.glow}`,cursor:"pointer",transition:"box-shadow .2s"}}
            onMouseEnter={e=>e.currentTarget.style.boxShadow=`0 0 40px ${t.glow}`}
            onMouseLeave={e=>e.currentTarget.style.boxShadow=`0 0 24px ${t.glow}`}>
            {group.icon}
          </div>
          <div style={{color:t.txt,fontSize:18,fontWeight:800,fontFamily:t.font,letterSpacing:".04em"}}>{group.name}</div>
          <div style={{color:t.txt2,fontSize:12,fontFamily:t.font,marginTop:3}}>{group.members.length} members · {group.members.filter(m=>m.status==="online").length} online</div>
          <div style={{display:"flex",gap:6,justifyContent:"center",marginTop:8,flexWrap:"wrap"}}>
            {group.tags.map(tag=>(
              <span key={tag} style={{background:t.tag,border:`1px solid ${t.tagBrd}`,borderRadius:12,padding:"2px 10px",fontSize:11,color:t.acc,fontFamily:t.font,fontWeight:600}}>{tag}</span>
            ))}
          </div>
        </div>
        {/* Tab bar */}
      ...
      {/* Content */}
      <div style={{flex:1,overflowY:"auto",padding:"12px 0",position:"relative",zIndex:1,scrollbarWidth:"thin",scrollbarColor:`${t.scrollbar} transparent`}}>

        {/* ── OVERVIEW ── */}
        {tab==="overview"&&(
          <div style={{padding:"0 12px",animation:"fadeUp .2s ease"}}>
            <PanelSection t={t} title="Group Name" icon={I.hash} defaultOpen>
              <EditField t={t} label="" value={group.name} onChange={v=>updateField("name",v)} placeholder="Enter group name…"/>
            </PanelSection>
            ...
            <PanelSection t={t} title="Invite Link" icon={I.link}>
              <div style={{display:"flex",gap:6,alignItems:"center",padding:"8px 12px",background:t.msgIn,border:`1px solid ${t.border}`,borderRadius:10}}>
                <span style={{flex:1,color:t.acc,fontSize:12,fontFamily:t.font,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{group.inviteLink}</span>
            ...
            <PanelSection t={t} title="Group Stats" icon={I.clock}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,padding:"4px 0"}}>
                {[["Messages",group.messageCount.toLocaleString()],["Members",group.members.length],["Created",group.createdAt],["Media",group.media.length+" files"]].map(([k,v])=>(
                  <div key={k} style={{background:t.msgIn,border:`1px solid ${t.border}`,borderRadius:10,padding:"10px 12px",textAlign:"center"}}>
                    <div style={{color:t.acc,fontSize:18,fontWeight:800,fontFamily:t.font}}>{v}</div>
                    <div style={{color:t.txt2,fontSize:11,fontFamily:t.font,marginTop:2}}>{k}</div>
                  </div>
                ))}
              </div>
            </PanelSection>
        ...
        {tab==="members"&&(
          <div style={{padding:"0 12px",animation:"fadeUp .2s ease"}}>
            <div style={{padding:"4px 4px 10px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{color:t.txt2,fontSize:12,fontFamily:t.font}}>{group.members.length} members</span>
              <Btn3D style={{padding:"5px 14px",borderRadius:8,background:t.acc,color:t.sendTxt,border:"none",fontSize:12,fontFamily:t.font,fontWeight:700}} onClick={()=>addToast("Invite sent (demo)")}>+ Invite</Btn3D>
            </div>
            {group.members.map(m=>(
        ...
        {tab==="media"&&(
          <div style={{padding:"0 12px",animation:"fadeUp .2s ease"}}>
            <div style={{padding:"4px 4px 10px",color:t.txt2,fontSize:12,fontFamily:t.font}}>{group.media.length} shared files</div>
            {/* Image grid */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:12}}>
              {group.media.filter(m=>m.type==="image").map((m,i)=>(
                <div key={i} style={{aspectRatio:"1",background:`linear-gradient(135deg,${t.acc}22,${t.acc2}11)`,borderRadius:10,border:`1px solid ${t.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,cursor:"pointer",transition:"border-color .15s"}}
            ...
            {group.media.filter(m=>m.type==="file").map((m,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:t.msgIn,border:`1px solid ${t.border}`,borderRadius:10,marginBottom:6,cursor:"pointer"}}
                onMouseEnter={e=>e.currentTarget.style.borderColor=t.acc}
                onMouseLeave={e=>e.currentTarget.style.borderColor=t.border}>
                <span style={{fontSize:20}}>📄</span>
                <div style={{flex:1}}><div style={{color:t.txt,fontSize:13,fontFamily:t.font}}>{m.name}</div><div style={{color:t.txt2,fontSize:11}}>{m.size}</div></div>
                <Ico d={I.download} size={15} stroke={t.txt2}/>
              </div>
            ))}
            {/* Links */}
            <div style={{color:t.txt,fontSize:13,fontWeight:700,fontFamily:t.font,padding:"10px 4px 6px",letterSpacing:".04em"}}>Shared Links</div>
            {group.links.map((l,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:t.msgIn,border:`1px solid ${t.border}`,borderRadius:10,marginBottom:6}}>
                <Ico d={I.link} size={13} stroke={t.acc}/>
                <span style={{color:t.acc,fontSize:12,fontFamily:t.font,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{l}</span>
          ...
          <div style={{padding:"0 12px",animation:"fadeUp .2s ease"}}>
            ...
            <PanelSection t={t} title="Restrictions" icon={I.shield} defaultOpen>
              <ToggleRow t={t} label="Slow Mode" sub={group.slowMode>0?`${group.slowMode}s delay`:"Off"} icon={I.clock}
                value={group.slowMode>0} onChange={on=>{const v=on?10:0;updateField("slowMode",v);}}/>
              <SelectRow t={t} label="Slow Mode Delay" icon={I.clock}
                options={[{v:0,l:"Off"},{v:5,l:"5 sec"},{v:10,l:"10 sec"},{v:30,l:"30 sec"},{v:60,l:"1 min"}]}
              ...
              <SelectRow t={t} label="Max Members" icon={I.users}
                ...
                value={group.maxMembers} onChange={v=>{updateField("maxMembers",Number(v));}}/>
              <SelectRow t={t} label="Disappearing Messages" icon={I.clock}
                options={[{v:0,l:"Off"},{v:3600,l:"1 Hour"},{v:86400,l:"24 Hours"},{v:604800,l:"7 Days"},{v:2592000,l:"30 Days"}]}
                value={group.disappearingMessages} onChange={v=>{updateField("disappearingMessages",Number(v));}}/>
            </PanelSection>
            <PanelSection t={t} title="Permissions" icon={I.crown}>
              {[["Send messages","Members can send messages"],["Send media","Members can send images/files"],["Add members","Members can invite others"],["Change info","Members can edit group info"]].map(([l,s],i)=>(
                <ToggleRow key={i} t={t} label={l} sub={s} value={i<2} onChange={()=>addToast(`${l} permission toggled`)}/>
              ))}
            </PanelSection>
          </div>
        )}
        ...
        {tab==="danger"&&(
          <div style={{padding:"0 12px",animation:"fadeUp .2s ease"}}>
            <div style={{background:"rgba(239,68,68,.05)",border:"1px solid rgba(239,68,68,.2)",borderRadius:12,padding:16,marginBottom:12}}>
              <div style={{color:"#ef4444",fontSize:13,fontWeight:700,fontFamily:t.font,marginBottom:8}}>⚠️ Danger Zone</div>
              <div style={{color:t.txt2,fontSize:12,fontFamily:t.font,lineHeight:1.6}}>Actions here are irreversible. Proceed with extreme caution.</div>
            </div>
            {[
              {label:"Clear all messages",sub:"Permanently delete chat history",icon:I.trash,color:"#f59e0b"},
              {label:"Reset invite link",sub:"Invalidate current invite link",icon:I.link,color:"#a855f7"},
              {label:"Transfer ownership",sub:"Give ownership to another member",icon:I.crown,color:"#3b82f6"},
              {label:"Archive group",sub:"Archive this nexus from active list",icon:I.download,color:"#f59e0b"},
              {label:"Delete group",sub:"Permanently destroy this nexus",icon:I.trash,color:"#ef4444"},
            ].map(({label,sub,icon,color})=>(
              <Btn3D key={label} onClick={()=>addToast(`${label} — confirmation required (demo)`)}
                style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:`${color}08`,border:`1px solid ${color}30`,borderRadius:12,marginBottom:8,textAlign:"left",cursor:"pointer",transition:"border-color .15s"}}
                onMouseEnter={e=>e?.currentTarget?.style&&(e.currentTarget.style.borderColor=color)}
                onMouseLeave={e=>e?.currentTarget?.style&&(e.currentTarget.style.borderColor=`${color}30`)}>
                <div style={{width:32,height:32,borderRadius:9,background:`${color}18`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Ico d={icon} size={15} stroke={color}/></div>
                <div style={{flex:1}}>
                  <div style={{color,fontSize:13,fontWeight:700,fontFamily:t.font}}>{label}</div>
      ...
      <div style={{background:t.header,borderTop:`1px solid ${t.border}`,flexShrink:0,position:"relative"}}>
        ...
        {recording&&(
          <div style={{padding:"10px 20px",background:t.msgIn,borderTop:`1px solid ${t.border}`,display:"flex",alignItems:"center",gap:14}}>
            ...
            <span style={{color:t.txt2,fontSize:12,fontFamily:t.font,marginLeft:"auto"}}>Tap mic to send · Tap again to cancel</span>
          </div>
        )}
...
export default function NexusChatDesktop() {
  const [activeTheme,setActiveTheme]=useState("vampire");
  const [groups,setGroups]=useState(()=>Object.fromEntries(THEME_KEYS.map(k=>[k,makeGroupState(k)])));
  const t=THEMES[activeTheme];
  const group=groups[activeTheme];
  const setGroup=useCallback((updater)=>{
    setGroups(g=>({...g,[activeTheme]:typeof updater==="function"?updater(g[activeTheme]):updater}));
  },[activeTheme]);

  // CSS vars for selection highlight
  const cssVars={
    "--sel":t.selection,
    "--acc":t.acc,
  };
    ...
    <>
      ...
      <div style={{width:"100vw",height:"100dvh",background:"#050505",display:"flex",flexDirection:"column",overflow:"hidden",...cssVars}}>
        ...
        <div style={{flexShrink:0,background:"#0a0a0a",borderBottom:"1px solid #1a1a1a",padding:"10px 24px",display:"flex",alignItems:"center",gap:8,overflowX:"auto",scrollbarWidth:"none"}}>
          <div style={{color:"#333",fontSize:10,letterSpacing:".25em",textTransform:"uppercase",marginRight:8,whiteSpace:"nowrap",fontFamily:"monospace"}}>NEXUSCHAT</div>
          <div style={{width:1,height:20,background:"#222",marginRight:8}}/>
          {THEME_KEYS.map(k=>{
            const th=THEMES[k];
            const active=activeTheme===k;
            return (
              <button key={k} onClick={()=>setActiveTheme(k)}
                style={{padding:"6px 18px",borderRadius:20,border:`1.5px solid ${active?th.acc:"#222"}`,background:active?`${th.acc}1a`:"transparent",color:active?th.acc:"#444",cursor:"pointer",fontSize:11,fontWeight:700,letterSpacing:".06em",textTransform:"uppercase",transition:"all .2s",whiteSpace:"nowrap",fontFamily:th.font,flexShrink:0,boxShadow:active?`0 0 20px ${th.glow2}`:"none"}}>
                {th.label}
              </button>
            );
          })}
          <div style={{flex:1}}/>
          <div style={{color:"#2a2a2a",fontSize:11,fontFamily:"monospace",letterSpacing:".08em",whiteSpace:"nowrap"}}>Desktop · {new Date().toLocaleTimeString("en",{hour:"2-digit",minute:"2-digit"})}</div>
        </div>

        {/* ── MAIN LAYOUT ── */}
        <div key={activeTheme} style={{flex:1,display:"flex",overflow:"hidden",animation:"fadeIn .3s ease"}}>

          {/* Narrow sidebar: theme nav indicator */}
          <div style={{width:60,background:t.sidebar,borderRight:`1px solid ${t.border}`,display:"flex",flexDirection:"column",alignItems:"center",padding:"16px 0",gap:12,flexShrink:0}}>
            <div style={{width:40,height:40,borderRadius:"50%",background:t.avatar,border:`2px solid ${t.acc}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,boxShadow:`0 0 18px ${t.glow}`,cursor:"pointer",fontFamily:t.font}}>{t.decoratorBig}</div>
            <div style={{width:32,height:1,background:t.border}}/>
            {[I.hash,I.bell,I.settings].map((icon,i)=>(
              <button key={i} style={{width:40,height:40,borderRadius:12,background:"transparent",border:"none",color:t.txt3,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"color .15s,background .15s"}}
                onMouseEnter={e=>{e.currentTarget.style.background=t.glow2;e.currentTarget.style.color=t.acc;}}
                onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color=t.txt3;}}>
                <Ico d={icon} size={17} stroke="currentColor"/>
              </button>
            ))}
          </div>

          {/* ── CHAT ── */}
          <ChatArea t={t} group={group} setGroup={setGroup} themeId={activeTheme}/>
        </div>
      </div>
    </>
  );
